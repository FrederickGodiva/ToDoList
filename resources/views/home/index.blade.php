                    ...
                    <tr class="bg-white border-b hover:bg-gray-100">
                        ...
                        <td class="py-4 text-center flex gap-1 justify-evenly">
                            <form action="/home/{{ $todo->id }}" method="post">
                                @csrf
                                @method('PUT')
                                <button type="submit" class="px-2 py-2 text-white bg-green-600 rounded-md" aria-label="Mark as Done">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-check">
                            ...
                            <form action="/home/{{ $todo->id }}" method="post">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="px-2 py-2 text-white bg-red-600 rounded-md" aria-label="Delete">
